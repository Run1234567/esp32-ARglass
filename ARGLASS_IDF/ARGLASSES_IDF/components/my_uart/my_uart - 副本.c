            ...
            switch(event.type) {
                case UART_DATA:
                    ...
                    // ==========================================
                        ...
                        ESP_LOGI(TAG, "📑 收到 UI 指令：请求章节列表 [%s] (偏移:%d)", param, offset);
                        scan_and_send_chapter_list(param, offset);
                    }
                    else if (strncmp((char*)dtmp, "CMD:READ_CHAP:", 14) == 0) {
                        char *rel_path = (char*)dtmp + 14;
                        ...
                        ESP_LOGI(TAG, "▶️ 准备阅读: %s", current_novel_path);
