...
static int auto_speed_idx = 9; // 默认挂在 1000ms (1�?
static int tts_speed_val = 4;  // 默认语�?4 (0-9)
...
static void update_novel_settings_display(void) {
    ...
    if (novel_read_mode == 0) {
        ...
        if (ms < 1000) sprintf(speed_str, "0.%d秒", ms / 100);
        else sprintf(speed_str, "%d秒", ms / 1000);
    } else {
        strcpy(mode_str, "语音同步");
        sprintf(speed_str, "语�? %d", tts_speed_val);
    }
...
void novel_ui_add_book(const char* name) {
    if (lvgl_port_lock(0)) {
        ...
        // �?当被选中时，变成深蓝色，非常显眼�?        lv_obj_set_style_bg_color(btn, lv_color_hex(0x0055AA), LV_STATE_FOCUSED); 
        lvgl_port_unlock();
    }
}
...
void novel_ui_add_chap(const char* name) {
    if (lvgl_port_lock(0)) {
        ...
        lv_obj_set_style_text_font(btn, &my_font_cn_16, 0);
        // �?当被选中时，变成深蓝�?        lv_obj_set_style_bg_color(btn, lv_color_hex(0x0055AA), LV_STATE_FOCUSED);
        lvgl_port_unlock();
    }
}

// ==========================================
//   �?my_uart.c 调用的添加分页按钮接�?// ==========================================
void novel_ui_add_book_page_btn(int is_next) {
    if (lvgl_port_lock(0)) {
        lv_obj_t * btn;
        if (is_next) {
            btn = lv_list_add_btn(novel_book_list, LV_SYMBOL_DOWN, "下一章");
            lv_obj_add_event_cb(btn, book_next_cb, LV_EVENT_CLICKED, NULL);
        } else {
            btn = lv_list_add_btn(novel_book_list, LV_SYMBOL_UP, "上一章");
...
void novel_ui_add_chap_page_btn(int is_next) {
    if (lvgl_port_lock(0)) {
        lv_obj_t * btn;
        if (is_next) {
            btn = lv_list_add_btn(novel_chap_list, LV_SYMBOL_DOWN, "下一章");
            lv_obj_add_event_cb(btn, chap_next_cb, LV_EVENT_CLICKED, NULL);
        } else {
            btn = lv_list_add_btn(novel_chap_list, LV_SYMBOL_UP, "上一章");
...
const char * test_novel_text =
    "第一�?陨落的天才\n\n"
    "“斗之力，三段！”\n\n"
    "望着测验魔石碑上面闪亮得甚至有些刺眼的五个大字，少年面无表情，唇角有着一抹自嘲，紧握的手掌，因为大力，而导致略微尖锐的指甲深深的刺进了掌心之中，带来一阵阵钻心的疼�?..\n\n"
