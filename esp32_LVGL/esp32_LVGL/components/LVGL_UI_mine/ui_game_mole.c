...
static void draw_holes(void) {
    for (int i = 0; i < 4; i++) {
        if (i == active_mole) {
            lv_obj_set_style_bg_color(holes[i], lv_color_hex(0xFF4444), 0);
            lv_obj_set_style_border_color(holes[i], lv_color_hex(0xFF0000), 0);
        } else {
            lv_obj_set_style_bg_color(holes[i], lv_color_black(), 0);
...
void ui_game_mole_init(void) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        lv_obj_set_size(holes[i], 40, 40);
        lv_obj_set_style_radius(holes[i], 20, 0);
